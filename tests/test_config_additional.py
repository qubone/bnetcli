...
def test_load_config_writes_default(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert isinstance(result, BnetConfig)
    assert "wine_prefix" in result.dict()
...
def test_build_environment_and_resolve_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    user_cfg = BnetConfig(wine_prefix=Path("auto"), environment=EnvironmentConfig(DXVK_HUD="1"))
    bcfg = BnetConfig.resolve(user_cfg)
